...
describe('TSMT Data Frame Analysis Tests', () =>
{
  ...
  const __frame2: TSMT$DataFrame = new TSMT$DataFrame();
  ...
  {
    let q: Array<number> = __stats.getQuantiles(__frame, "price", 0.25);
  ...
  {
    let q: Array<number> = __stats.getQuantiles(__frame, "price", 0.2);
  ...
  {
    let median: number = __stats.singleStat(__frame, "mileage", SingleStatEnum.MEDIAN);

    expect(median).to.equal(36385);

    median = __stats.singleStat(__frame2, "Mileage", SingleStatEnum.MEDIAN);
  ...
  {
    const obj: Object        = __analysis.oneWayTable(__frame2, "Year");
    const tbl: Array<Object> = __analysis.toArray(obj);
  ...
  {
    const output: Object = __analysis.crossTable(__frame2, "Model", "Color",
      ["Black Silver White Gray", "Blue Gold Green Red Yellow"],
      ["Simple-Color", "Bold-Color"]);
